...
#modelCheckPoint
modelCheckpoint = ModelCheckpoint(filepath = 'inceptionV3_melanoma_' +str(input_size) +'x' + str(input_size) + '.h5',
                                  monitor='val_loss',
                                  verbose=1,
                                  save_best_only=True,
...
history = model.fit(x_train, y_train, \
    ...
    verbose = 1, \
    callbacks=[lr_decay, early_stopping, modelCheckpoint])
...
#load model
model=load_model('inceptionV3_melanoma_' +str(input_size) +'x' + str(input_size) + '.h5')
